...
station_information_df = pd.concat(station_information_df_list, axis=0, sort=False)[["capacity","eightd_has_key_dispenser","lat","lon","name","region_id","rental_methods","short_name","station_id","address","station_code","post_code","external_id","has_kiosk","rental_url","cross_street","eightd_station_services","dockless_bikes_parking_zone","dockless_bikes_parking_zone_capacity"]]
...
system_pricing_df = pd.concat(system_pricing_df_list, axis=0, sort=False)[["currency","description","is_taxable","name","plan_id","price","taxable","url"]]
